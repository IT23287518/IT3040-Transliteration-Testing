...
def run_test():
    ...
    input_col_idx = 3      # Column C (Input)
    expected_col_idx = 4   # Column D (Expected output)
    actual_col_idx = 5     # Column E (Actual output)
    status_col_idx = 6     # Column F (Status)
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                def attempt_row(wait_ms_extra=0):
                    prev_out = _read_output(is_chat, output_locator)
                    
                    # 1. Clear input field using CTRL+A and Backspace
                    try:
                        input_locator.click(timeout=2000)
                        page.keyboard.press("Control+A")
                        page.keyboard.press("Backspace")
                    ...
                    # 2. Wait until output area becomes empty or changes from the previous output
                    for _ in range(10):
                        current_out = _read_output(is_chat, output_locator)
                        if not current_out or current_out != prev_out:
                            break
                        page.wait_for_timeout(300)
                        
                    # Type the input
                    if int(args.type_delay_ms) > 0:
                        try:
                            input_locator.click(timeout=2000)
                            input_locator.type(singlish_input, delay=int(args.type_delay_ms))
                        except Exception:
                            input_locator.fill(singlish_input)
                    else:
                        input_locator.fill(singlish_input)
                    ...
                    # 3. Wait longer for the translated output to update
                    page.wait_for_timeout(max(0, int(args.wait_ms)) + wait_ms_extra)

                    actual_out = ""
                    current_out = ""
                    tries = max(1, int(args.retries)) + (wait_ms_extra // 1000)
                    for i in range(tries):
                        current_out = _read_output(is_chat, output_locator)
                        if not current_out:
                            page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                            continue
                        if prev_out and current_out == prev_out:
                            page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                            continue
                        if current_out:
                            actual_out = current_out
                            break
                        page.wait_for_timeout(max(0, int(args.retry_wait_ms)))
                    
                    return actual_out, current_out

                actual_output, current_output = attempt_row(wait_ms_extra=2000)
                
                # 8. Retry logic
                if not actual_output:
                    print("  -> Output did not update. Retrying...")
                    actual_output, current_output = attempt_row(wait_ms_extra=5000)

                # 4, 6, 8. If output doesn't update, write error message and mark FAIL
                if not actual_output:
                    actual_output = "Output did not update / UI interaction error"
                    status = "FAIL"
                else:
                    # 5. Always write PASS or FAIL into the Status column
                    if expected_sinhala:
                        status = "PASS" if actual_output.strip() == expected_sinhala.strip() else "FAIL"
                    else:
                        status = "PASS"
                ...
                # 7. Save the Excel file after every tested row
                try:
                    wb.save(args.output)
                except Exception as e:
                    print(f"  -> Warning: Failed to save Excel file for row {row_index}: {e}")
                
            except Exception as e:
                print(f"Error in UI interaction: {e}")
                try:
                    _set_cell_value(ws, row_index, actual_col_idx, "Output did not update / UI interaction error")
